...
export type CatalogTargetPreviewInput = {
  name: string;
...
type CatalogTargetsPreviewModalProps = {
  open: boolean;
  target: CatalogTargetPreviewInput | null;
  onClose: () => void;
  entityNoun?: "offer" | "campaign";
};
...
export function CatalogTargetsPreviewModal({
  open,
  target,
  onClose,
  entityNoun = "offer"
}: CatalogTargetsPreviewModalProps) {
  ...
  }, [open, target, reloadToken, loadProductsList, loadCategoriesList, loadMissingProductDetails]);
  ...
  const isProductScope = target.scope === "product";
  const title = isProductScope ? "Targeted Products" : "Targeted Categories";
  const titleId = "catalog-targets-preview-title";
  const empty =
    !loading &&
    !error &&
    (isProductScope ? target.productIds.length === 0 : target.categoryIds.length === 0);
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-1 truncate text-sm text-foreground/60">{target.name}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {loading ? (
            ...
            <p className="py-8 text-center text-sm text-foreground/60">
              This {entityNoun} has no {isProductScope ? "product" : "category"} targets.
            </p>
          ) : isProductScope ? (
