        ...
        {STAR_LEVELS.map((level) => {
          ...
          return (
            <div key={level} className="flex items-center gap-2 text-xs">
              <span className="w-6 shrink-0 font-medium text-[#666666]">{level}★</span>
              <div className="h-2 flex-1 overflow-hidden rounded-full bg-[#FFF0F4]">
                <div
                  className="h-full rounded-full bg-secondary transition-all duration-500"
                  style={{ width: `${width}%` }}
                />
              </div>
              <span className="w-6 shrink-0 text-right text-[#888888]">{count}</span>
            </div>
          );
        })}
      </div>
